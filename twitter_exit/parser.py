...
def tweet_json_to_markdown(tweet, username, archive_media_folder, output_media_folder_name):
    ...
    timestamp = int(round(datetime.datetime.strptime(timestamp_str,
                                                     '%a %b %d %X %z %Y').timestamp()))  # Example: Tue Mar 19 14:05:17 +0000 2019
    ...
    if 'entities' in tweet and 'media' in tweet['entities'] and 'extended_entities' in tweet and 'media' in tweet[
        'extended_entities']:
            ...
            if 'url' in media and 'media_url' in media:
                ...
                if os.path.isfile(local_filename):
                    ...
                    if len(media_filenames) > 0:
                        ...
                        print(
                            f'Warning: missing local file: {local_filename}. Using original link instead: {original_url} (expands to {original_expanded_url})')
...
def main():
    ...
    if not os.path.isfile(account_js_filename):
        print(
            f'Error: Failed to load {account_js_filename}. Start this script in the root folder of your Twitter archive.')
        exit()
    ...
    for tweets_js_filename in input_filenames:
        ...
        tweets_markdown += [tweet_json_to_markdown(tweet, username, archive_media_folder, output_media_folder_name) for
                            tweet in json]
